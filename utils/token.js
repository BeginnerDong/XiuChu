...
class Token {
	...
	getTokenFromServer(wxUserInfo, params, callback) {
		...
		wx.login({
			success: function(res) {
				...
				wx.request({
					url: 'https://www.xiuchukj.com/api/public/index.php/api/v1/Base/ProgramToken/get',
					method: 'POST',
					data: postData,
					success: function(res) {
						console.log(res)
	...
	getToken(callback, params) {
		...
		if (wx.getStorageSync('login').login_name && wx.getStorageSync('login').password) {
			...
			wx.request({
				url: 'https://www.xiuchukj.com/api/public/index.php/api/v1/Func/Common/loginByUp',
				method: 'POST',
				data: postData,
				success: function(res) {
					console.log(res)
